...
int getIndex_lap(int row, int col, int cols) {
    return row * cols + col;
}


double getValue_lap(float* input, int row, int col, int rows, int cols) {
    if (row < 0) row = 0;
    if (row >= rows) row = rows - 1;
    if (col < 0) col = 0;
    if (col >= cols) col = cols - 1;

    return input[getIndex_lap(row, col, cols)];
}

// 进行拉普拉斯滤波
void applyLaplacianFilter(float* input, float* output, int rows, int cols) {
    int i, j, m, n;
    int kernelSize = 3;
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            ...
            for (m = -RADIUS_LAP; m <= RADIUS_LAP; m++) {
                for (n = -RADIUS_LAP; n <= RADIUS_LAP; n++) {
                    int x = i + m;
                    int y = j + n;


                    sum += getValue_lap(input, x, y, rows, cols) * laplacian[m + RADIUS_LAP][n + RADIUS_LAP];
                }
            }

            output[getIndex(i, j, cols)] = sum;
...
void all_iteration(int myid,int np,int sy,int gy,MPI_Status status,int **table,modelpar *model,float *image_pp,float *image_ps,float *pp_grad,float *ps_grad,float *illumination,int maxiter,int record_left_in_v){
...
//			fflush(stdout);
    ...
    for(int iter=0;iter<maxiter;iter++)
    {
		...
		for(;;)
		{
			...
			IMAGE_SINGLESHOT(image2delastic,sno,Dim,myid,model->record_z,model->record_x,model->gc,model->rbc,model->cpu_mem,model->iointerval,iter,model->imagedir,image_pp,image_ps,pp_grad,ps_grad,misfit);
...
// pre-condition using laplace
		// laplace_filter_2d(model->allnz,model->allnx,pp_grad);    
		// laplace_filter_2d(model->allnz,model->allnx,ps_grad);            
//

            // normalize_all(pp_grad, ps_grad, model->allnz, model->allnx);        

            for(int i=0;i<model->allnx;i++)
                for(int j=0;j<model->allnz;j++)
                {
                    if(j>=26){
                        // pp_cg[i*model->allnz+j] = pp_grad[i*model->allnz+j]/illumination[i*model->allnz+j];
                        // ps_cg[i*model->allnz+j] = ps_grad[i*model->allnz+j]/illumination[i*model->allnz+j]; 
                        // pp_cg[i*model->allnz+j] = pp_grad[i*model->allnz+j];
                        // ps_cg[i*model->allnz+j] = ps_grad[i*model->allnz+j];     

                        sum_pp_grad[i*model->allnz+j] = b_pp_grad[i*model->allnz+j] + b_pp_grad2[i*model->allnz+j];
                        sum_ps_grad[i*model->allnz+j] = b_ps_grad[i*model->allnz+j] + b_ps_grad2[i*model->allnz+j];     
                    ...
                    beta_fenzi += pp_grad[i*model->allnz+j]*pp_grad[i*model->allnz+j]/illumination[i*model->allnz+j] + ps_grad[i*model->allnz+j]*ps_grad[i*model->allnz+j]/illumination[i*model->allnz+j]; 
                    // beta_fenzi += pp_grad[i*model->allnz+j]*pp_grad[i*model->allnz+j]+ ps_grad[i*model->allnz+j]*ps_grad[i*model->allnz+j];                     
                }
            ...
            // if(iter>10){            // first cg  after steepest
            //     beta = 0;
            //     std::cout<<"Start using steepesd instead of CG.\n"<<"CG --------> STEEPEST"<<std::endl;
            // }
            ...
            for(int i=0;i<model->allnx;i++)
                for(int j=0;j<model->allnz;j++)
                {
                    // pp_cg[i*model->allnz+j] = pp_grad[i*model->allnz+j] + beta*pp_cg[i*model->allnz+j];                 
                    // ps_cg[i*model->allnz+j] = ps_grad[i*model->allnz+j] + beta*ps_cg[i*model->allnz+j]; 
            ...
            sprintf(imagepath_pp, "%s/sum_cggrad_pp_%d_%d_%d.dat",model->imagedir,iter,model->allnx,model->allnz);
            fpp = fopen(imagepath_pp,"wb");
            fwrite(pp_cg,model->allnx*model->allnz*sizeof(float),1,fpp);
            fclose(fpp);  

            sprintf(imagepath_pp, "%s/sum_cggrad_ps_%d_%d_%d.dat",model->imagedir,iter,model->allnx,model->allnz);
            fpp = fopen(imagepath_pp,"wb");
            fwrite(ps_cg,model->allnx*model->allnz*sizeof(float),1,fpp);
            fclose(fpp);  

            sprintf(imagepath_pp, "%s/sum_bgrad_pp_%d_%d_%d.dat",model->imagedir,iter,model->allnx,model->allnz);
            fpp = fopen(imagepath_pp,"wb");
            fwrite(b_pp_grad,model->allnx*model->allnz*sizeof(float),1,fpp);
            fclose(fpp);  

            sprintf(imagepath_pp, "%s/sum_bgrad_ps_%d_%d_%d.dat",model->imagedir,iter,model->allnx,model->allnz);
            fpp = fopen(imagepath_pp,"wb");
            fwrite(b_ps_grad,model->allnx*model->allnz*sizeof(float),1,fpp);
            fclose(fpp);              
            ...
            sprintf(imagepath_pp, "%s/sum_illum_%d_%d_%d.dat",model->imagedir,iter,model->allnx,model->allnz);
            fpp = fopen(imagepath_pp,"wb");
            fwrite(illumination,model->allnx*model->allnz*sizeof(float),1,fpp);
            fclose(fpp);  
...
//calculate alpha &  update m
		...
		for(;;)
		{
			...
			CALCULATE_ALPHA(image2delastic,sno,Dim,myid,model->record_z,model->record_x,model->gc,model->imagedir,model->rbc,model->cpu_mem,model->iointerval,iter,image_pp,image_ps,pp_cg,ps_cg,fenzi,fenmu);
//calculating instead of memory
		...
		if(myid==1){
            ...
            for(int i=0;i<model->allnx;i++)
                for(int j=0;j<model->allnz;j++)
                {
                   image_pp[i*model->allnz+j] = (image_pp[i*model->allnz+j] - alpha*pp_cg[i*model->allnz+j]);
                   image_ps[i*model->allnz+j] = (image_ps[i*model->allnz+j] - alpha*ps_cg[i*model->allnz+j]);
                }            
            char imagepath_pp[1024];
...
int main(int argc,char *argv[]){
    ...
    all_iteration(myid,np,sy,gy,status,table,&model,image_pp,image_ps,pp_grad,ps_grad,illumination,maxiter,record_left_in_v);
