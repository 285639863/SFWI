...
__global__ void update_txx_tzz_txz_born(float *txx, float *tzz,float *txx_x,float *txx_z,float *tzz_x,float *tzz_z,float *txz,float *txz_x,float *txz_z,float *vx,float *vz,\
    float *f_vx, float *f_vz, float *delta_mp,float *delta_ms,const int nxpml, const int nzpml,const float dt,const float dx,const float dz,const int nop,float *lamda,float *miu,float *vp,float *vs,float *rho,float* dampx,float* dampz,int direction)
{
...
//			tmp_vz += coeff2[i]*(vz[ix*nzpml+(iz+i-1)]-vz[ix*nzpml+(iz-i)]);
			 ...
			 tmp_f_vx_x += coeff2[i]*(f_vx[(ix+i-1)*nzpml+iz]-f_vx[(ix-i)*nzpml+iz]);
			 tmp_f_vz_z += coeff2[i]*(f_vz[ix*nzpml+(iz+i-1)]-f_vz[ix*nzpml+(iz-i)]);	

			 tmp_f_vx_z += coeff2[i]*(f_vx[ix*nzpml+(iz+i)]-f_vx[ix*nzpml+(iz-i+1)]);
			 tmp_f_vz_x += coeff2[i]*(f_vz[(ix+i)*nzpml+iz]-f_vz[(ix-i+1)*nzpml+iz]);				 			 		
		}

// use refl vp and vs 
		 txx_x[index] = (damp1*txx_x[index]+direction*dt*((lamda[index]+2*miu[index])*tmp_vx_x/dx  + (2*(lamda[index]+2*miu[index])*(delta_mp[index])*(tmp_f_vx_x)/dx)))/damp2;
		 txx_z[index] = (damp3*txx_z[index]+direction*dt*(lamda[index]*tmp_vz_z/dz + (2*(lamda[index]+2*miu[index])*(delta_mp[index])-2*miu[index]*(delta_ms[index]))*(tmp_f_vz_z)/dz))/damp4;
		 tzz_x[index] = (damp1*tzz_x[index]+direction*dt*(lamda[index]*tmp_vx_x/dx + (2*(lamda[index]+2*miu[index])*(delta_mp[index])-2*miu[index]*(delta_ms[index]))*(tmp_f_vx_x)/dx))/damp2;
		 tzz_z[index] = (damp3*tzz_z[index]+direction*dt*((lamda[index]+2*miu[index])*tmp_vz_z/dz + 2*(lamda[index]+2*miu[index])*(delta_mp[index])*(tmp_f_vz_z)/dz))/damp4;

		 txz_x[index] = (damp1*txz_x[index]+direction*dt*(miu[index]*tmp_vz_x/dx + 2*miu[index]*(delta_ms[index])*(tmp_f_vz_x)/dx))/damp2;
		 txz_z[index] = (damp3*txz_z[index]+direction*dt*(miu[index]*tmp_vx_z/dz + 2*miu[index]*(delta_ms[index])*(tmp_f_vx_z)/dz))/damp4;

// use delta vp and vs 
		//  txx_x[index] = (damp1*txx_x[index]+direction*dt*((lamda[index]+2*miu[index])*tmp_vx_x/dx  \
		//  + (2*(rho[index]*vp[index])*(delta_mp[index])*(tmp_f_vx_x)/dx)))/damp2;

		//  txx_z[index] = (damp3*txx_z[index]+direction*dt*(lamda[index]*tmp_vz_z/dz \
		//  + (2*(rho[index]*vp[index])*(delta_mp[index]) - 4*(rho[index]*vs[index])*(delta_ms[index]))*(tmp_f_vz_z)/dz))/damp4;

		//  tzz_x[index] = (damp1*tzz_x[index]+direction*dt*(lamda[index]*tmp_vx_x/dx \
		//  + (2*(rho[index]*vp[index])*(delta_mp[index]) - 4*(rho[index]*vs[index])*(delta_ms[index]))*(tmp_f_vx_x)/dx))/damp2;

		//  tzz_z[index] = (damp3*tzz_z[index]+direction*dt*((lamda[index]+2*miu[index])*tmp_vz_z/dz \
		//  + 2*(rho[index]*vp[index])*(delta_mp[index])*(tmp_f_vz_z)/dz))/damp4;

		//  txz_x[index] = (damp1*txz_x[index]+direction*dt*(miu[index]*tmp_vz_x/dx \
		//  + 2*(rho[index]*vs[index])*(delta_ms[index])*(tmp_f_vz_x)/dx))/damp2;

		//  txz_z[index] = (damp3*txz_z[index]+direction*dt*(miu[index]*tmp_vx_z/dz \
		//  + 2*(rho[index]*vs[index])*(delta_ms[index])*(tmp_f_vx_z)/dz))/damp4;
