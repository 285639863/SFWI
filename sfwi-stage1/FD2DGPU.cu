...
void FD2DGPU_ELASTIC::GPUbufferVPVS(float *vp_all,float *vs_all,float *cpu_rho_all){
		...
		cudaMemcpy(rho_all,cpu_rho_all,allnx*allnz*sizeof(float),cudaMemcpyHostToDevice);		
}
...
void FD2DGPU_ELASTIC::bufferVpVsHtoD(const int ngx_min){
	...
	copyvelocityHtoD<<<Grid_Block.grid,Grid_Block.block>>>(rho_all,d_rho,pml,nzpml,ngx_min,allnz,nx,nz);
	...
	copypmlVz<<<Grid_Block.grid,Grid_Block.block>>>(d_rho,pml,nxpml,nzpml);
	copypmlVx<<<Grid_Block.grid,Grid_Block.block>>>(d_rho,pml,nxpml,nzpml);
...
// 
	...
	cudaMalloc((void**)&rho_all,nxnz*sizeof(float));	
...
//	cudaMemset(receiver2,0,ntrace*nt*sizeof(float));			
	...
	cudaMemset(rho_all,0,nxnz*sizeof(float));	
...
void FD2DGPU_ELASTIC::born_modeling(int direction,int forward_or_born){
...
// =3 top part born before low pass
	if(forward_or_born == 3){
		add_born_source<<<Grid_Block.grid,Grid_Block.block>>>(vx,vz,b_txx,b_tzz,b_txz,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,nop,lamda,miu,d_rho,vp,vs,direction);
		update_vx<<<Grid_Block.grid,Grid_Block.block>>>(b_txx,b_txz,b_vx,b_vx_x,b_vx_z,nxpml,nzpml,dt,dx,dz,nop,d_rho,d_damp,d_dampz,direction);    
		update_vz<<<Grid_Block.grid,Grid_Block.block>>>(b_tzz,b_txz,b_vz,b_vz_x,b_vz_z,nxpml,nzpml,dt,dx,dz,nop,d_rho,d_damp,d_dampz,direction);
	}
...
//		add_born_source<<<Grid_Block.grid,Grid_Block.block>>>(b_vx,b_vz,b2_txx,b2_tzz,b2_txz,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,nop,lamda,miu,d_rho,vp,vs,direction);
		...
		// add_born_source_components<<<Grid_Block.grid,Grid_Block.block>>>(b_vx,b_vz,b2_txx_x,b2_txx_z,b2_tzz_x,b2_tzz_z,b2_txz_x,b2_txz_z,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,dt,nop,lamda,miu,d_rho,vp,vs,direction);
		...
		update_vx_vz<<<Grid_Block.grid,Grid_Block.block>>>(b2_txx,b2_tzz,b2_txz,b2_vx,b2_vx_x,b2_vx_z,b2_vz,b2_vz_x,b2_vz_z,nxpml,nzpml,dt,dx,dz,nop,d_rho,d_damp,d_dampz,direction);    
		add_born_source_components<<<Grid_Block.grid,Grid_Block.block>>>(b_vx,b_vz,b2_txx_x,b2_txx_z,b2_tzz_x,b2_tzz_z,b2_txz_x,b2_txz_z,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,dt,nop,lamda,miu,d_rho,vp,vs,direction);
		update_txx_tzz_txz<<<Grid_Block.grid,Grid_Block.block>>>(b2_txx,b2_tzz,b2_txx_x,b2_txx_z,b2_tzz_x,b2_tzz_z,b2_txz,b2_txz_x,b2_txz_z,b2_vx,b2_vz,nxpml,nzpml,dt,dx,dz,nop,lamda,miu,d_damp,d_dampz,direction);
...
// =7 reconstruct scatter wavefield 
	if(forward_or_born == 7 ){

		add_born_source_components<<<Grid_Block.grid,Grid_Block.block>>>(vx,vz,b_txx_x,b_txx_z,b_tzz_x,b_tzz_z,b_txz_x,b_txz_z,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,dt,nop,lamda,miu,d_rho,vp,vs,direction);
		update_txx_tzz_txz_grad<<<Grid_Block.grid,Grid_Block.block>>>(b_txx,b_tzz,b_txx_x,b_txx_z,b_tzz_x,b_tzz_z,b_txz,b_txz_x,b_txz_z,b_vx,b_vz,b_vx_gx,b_vz_gz,b_vx_gz,b_vz_gx,nxpml,nzpml,dt,dx,dz,nop,lamda,miu,d_damp,d_dampz,direction,pml);
		update_vx_vz<<<Grid_Block.grid,Grid_Block.block>>>(b_txx,b_tzz,b_txz,b_vx,b_vx_x,b_vx_z,b_vz,b_vz_x,b_vz_z,nxpml,nzpml,dt,dx,dz,nop,d_rho,d_damp,d_dampz,direction);    
...
// =9 adjoint scatter wavefield 
	if(forward_or_born == 9 ){
		add_born_source_adjoint_components<<<Grid_Block.grid,Grid_Block.block>>>(txx_bk,tzz_bk,txz_bk,b_vx_bk,b_vz_bk,b_vx_x_bk,b_vx_z_bk,b_vz_x_bk,b_vz_z_bk,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,dt,nop,lamda,miu,d_rho,vp,vs,direction);	
		GPUcalculate_elastic_vx_vz_back<<<Grid_Block.grid,Grid_Block.block>>>(b_txx_bk,b_tzz_bk,b_txz_bk,b_vx_bk,b_vx_x_bk,b_vx_z_bk,b_vz_bk,b_vz_x_bk,b_vz_z_bk,lamda,miu,nxpml,nzpml,dt,dx,dz,nop,d_rho,d_damp,d_dampz,direction);		
		GPUcalculate_elastic_txx_tzz_txz_back<<<Grid_Block.grid,Grid_Block.block>>>(b_txx_bk,b_tzz_bk,b_txx_x_bk,b_txx_z_bk,b_tzz_x_bk,b_tzz_z_bk,b_txz_bk,b_txz_x_bk,b_txz_z_bk,b_vx_bk,b_vz_bk,nxpml,nzpml,dt,dx,dz,nop,lamda,miu,d_damp,d_dampz,direction);			
	}
...
void FD2DGPU_ELASTIC::pure_born_reconstruct_backward(int direction,int forward){
		...
		if(forward){
			...
			// add_born_source<<<Grid_Block.grid,Grid_Block.block>>>(vx,vz,b_txx,b_tzz,b_txz,s_image_pp_m,s_image_ps_m,nxpml,nzpml,pml,dx,dz,nop,lamda,miu,d_rho,vp,vs,1);				// TODO: if need and -1 or 1
...
__global__ void GPUrecord_scatterwavefield(float *sz,float *sx,float* scatter_record_z,float* scatter_record_x,int* gc,const int it,const int ngx_left,const int pml,const int nzpml,const int ntr,const float dx,const int nt,const int igz)
{
    const int index = blockIdx.x*blockDim.x+threadIdx.x;
...
__global__ void GPUrecord_scatterwavefield_new(float *vz,float *vx,float *sz,float *sx,float* scatter_record_z,float* scatter_record_x,int* gc,const int it,const int ngx_left,const int pml,const int nzpml,const int ntr,const float dx,const int nt,const int igz)
{
    const int index = blockIdx.x*blockDim.x+threadIdx.x;
	if(index>ntr-1)return;
	int igx = pml + (int)((float)(gc[index]-ngx_left)/dx);
	scatter_record_z[(index*nt)+it] = vz[igx*nzpml+igz] + sz[igx*nzpml+igz];
	scatter_record_x[(index*nt)+it] = vx[igx*nzpml+igz] + sx[igx*nzpml+igz];		
...
void FD2DGPU_ELASTIC::subtract_ingrad(double alpha,bool doublescatter){
	dim3 gridsize((nt+BLOCKDIMX-1)/BLOCKDIMX,(ntr+BLOCKDIMY-1)/BLOCKDIMY);
	dim3 blocksize(BLOCKDIMX,BLOCKDIMY);
	...
	if(!doublescatter){
		...
		GPU_subtract<<<gridsize,blocksize>>>(GPUrecord_scatter2_z,GPUrecord_scatter2_x,GPUrecord,GPUrecord_x,nt,ntr,alpha);		
	}
}
...
__global__ void sum_gradient(float *pp_u0g0,float *ps_u0g0,float *pp_u1g0,float *ps_u1g0,float *pp_u0g1,float *ps_u0g1,const int nx,const int nz,const int delta_left,const int record_left_in_v){
	int iz = blockIdx.x*blockDim.x+threadIdx.x;
	int ix = blockIdx.y*blockDim.y+threadIdx.y;
	if(ix>nx-1||iz>nz-1)return;

	pp_u0g0[(delta_left+record_left_in_v+ix)*nz+iz] = pp_u1g0[(delta_left+record_left_in_v+ix)*nz+iz] + pp_u0g1[(delta_left+record_left_in_v+ix)*nz+iz];
	ps_u0g0[(delta_left+record_left_in_v+ix)*nz+iz] = ps_u1g0[(delta_left+record_left_in_v+ix)*nz+iz] + ps_u0g1[(delta_left+record_left_in_v+ix)*nz+iz];
...
// 	float cross_correlation_ps = -2*vs[(ix+pml)*nzpml+iz+pml]*vs[(ix+pml)*nzpml+iz+pml]*rho[(ix+pml)*nzpml+iz+pml]*miu_grad;
...
// use refl vp and vs
	// float cross_correlation_pp = 2*vp[(ix+pml)*nzpml+iz+pml]*vp[(ix+pml)*nzpml+iz+pml]*rho[(ix+pml)*nzpml+iz+pml]*lamda_grad;
	// float cross_correlation_ps = -4*vs[(ix+pml)*nzpml+iz+pml]*vs[(ix+pml)*nzpml+iz+pml]*rho[(ix+pml)*nzpml+iz+pml]*lamda_grad + 2*vs[(ix+pml)*nzpml+iz+pml]*vs[(ix+pml)*nzpml+iz+pml]*rho[(ix+pml)*nzpml+iz+pml]*miu_grad;
...
// use delta vp and vs
	...
	allimageGPU_numerator_pp[(delta_left+record_left_in_v+ix)*nz+iz] += scale*cross_correlation_pp;
	allimageGPU_numerator_ps[(delta_left+record_left_in_v+ix)*nz+iz] += scale*cross_correlation_ps;	
...
__global__ void GPUapplysource_backward_from_record(float *vz,float *vx,float* record,float *record_x,int* gc,const int it,const float ngx_left,const int pml,const int nzpml,\
	const int ntr,const float dx,const int nt,bool stack,int igz)
{
    const int index = blockIdx.x*blockDim.x+threadIdx.x;
	if(index>ntr-1)return;
	int igx = pml + (int)((float)(gc[index]-ngx_left)/dx);
...
//    GPUapplysource_backward_from_record<<<adds_bac.grid,adds_bac.block,0,stream>>>(vz_bk,vx_bk,GPUrecord,GPUrecord_x,GPUgc,it,cmin,pml,nzpml,ntr,dx,nt);	
    GPUapplysource_backward_from_record<<<adds_bac.grid,adds_bac.block,0,stream>>>(vz_bk,vx_bk,GPUrecord_scatter_z,GPUrecord_scatter_x,GPUgc,it,ngx_left,pml,nzpml,ntr,dx,nt,stack,igz);		
}
...
void FD2DGPU_ELASTIC::image_gradient(cudaStream_t stream,bool double_scatter){
...
void FD2DGPU_ELASTIC::hessian_vector_product(){

	doimage_gradient<<<imagesize.grid,imagesize.block>>>(pp_gradient,ps_gradient,vx_gx,vz_gz,vx_gz,vz_gx,txx_bk,tzz_bk,txz_bk,vp,vs,d_rho,nx,nz,delta_left,record_left_in_v,pml,nzpml);	// vpx_g vpz_g

}


void FD2DGPU_ELASTIC::hessian_vector_product_misfit(){

	doimage_gradient<<<imagesize.grid,imagesize.block>>>(b_allimageGPU_numerator_pp,b_allimageGPU_numerator_ps,vx_gx,vz_gz,vx_gz,vz_gx,txx_bk,tzz_bk,txz_bk,vp,vs,d_rho,nx,nz,delta_left,record_left_in_v,pml,nzpml);	// vpx_g vpz_g

}


void FD2DGPU_ELASTIC::hessian_vector_product_mb(){

	// doimage_gradient<<<imagesize.grid,imagesize.block>>>(pp_gradient,ps_gradient,vx_gx,vz_gz,vx_gz,vz_gx,txx_bk,tzz_bk,txz_bk,vp,vs,d_rho,nx,nz,delta_left,record_left_in_v,pml,nzpml);	// vpx_g vpz_g

	doimage_gradient<<<imagesize.grid,imagesize.block>>>(b_allimageGPU_numerator_pp,b_allimageGPU_numerator_ps,b_vx_gx,b_vz_gz,b_vx_gz,b_vz_gx,\
	txx_bk,tzz_bk,txz_bk,vp,vs,d_rho,nx,nz,delta_left,record_left_in_v,pml,nzpml);			// forward scatter G1 * receiver delta_d G0

	doimage_gradient<<<imagesize.grid,imagesize.block>>>(allimageGPU_numerator_pp,allimageGPU_numerator_ps,vx_gx,vz_gz,vx_gz,vz_gx,\
	b_txx_bk,b_tzz_bk,b_txz_bk,vp,vs,d_rho,nx,nz,delta_left,record_left_in_v,pml,nzpml);			// forward G0 * receiver scatter G1

}

void FD2DGPU_ELASTIC::add_gradient(){

	sum_gradient<<<imagesize.grid,imagesize.block>>>(pp_gradient,ps_gradient,allimageGPU_numerator_pp,allimageGPU_numerator_ps,b_allimageGPU_numerator_pp,b_allimageGPU_numerator_ps,
	nx,nz,delta_left,record_left_in_v);
}
...
void FD2DGPU_ELASTIC::record_scatter_new(const int it,bool double_scatter){
	dim3 gridsize((ntr+BLOCKDIMX-1)/BLOCKDIMX,1);
	dim3 blocksize(BLOCKDIMX,1);
	GPUrecord_scatterwavefield_new<<<gridsize,blocksize>>>(vz,vx,b_vz,b_vx,GPUrecord_scatter_z,GPUrecord_scatter_x,GPUgc,it,ngx_left,pml,nzpml,ntr,dx,nt,igz);

}



void FD2DGPU_ELASTIC::record_background_wave(const int it){
	dim3 gridsize((ntr+BLOCKDIMX-1)/BLOCKDIMX,1);
	dim3 blocksize(BLOCKDIMX,1);
	GPUrecord_scatterwavefield<<<gridsize,blocksize>>>(vz,vx,GPUrecord_scatter_z,GPUrecord_scatter_x,GPUgc,it,ngx_left,pml,nzpml,ntr,dx,nt,igz);
}
...
template<typename T>
__global__ void fft_shift(T *data, int width, int height) {
    ...
    if (x < width / 2 && y < height / 2) {
        ...
        int index2 = y * width + (x + width / 2);
        int index3 = (y + height / 2) * width + x;
        int index4 = (y + height / 2) * width + (x + width / 2);
...
template<typename T>
__global__ void ifft_shift(T *data, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
template<typename T>
void construct_gaussian_bandpass_filter(T *filter, int width, int height, int low_cutoff, int high_cutoff) {
    int cx = width / 2;
    int cy = height / 2;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            T distance = sqrt(dx * dx + (scale*dy) * (scale*dy));
			T gaussian = 0;
			if(low_cutoff==0){
            	gaussian = exp(-distance * distance / (2.0 * high_cutoff * high_cutoff));		// lowpass filter		
			}
			else{
            	gaussian = exp(-distance * distance / (2.0 * high_cutoff * high_cutoff)) - exp(-distance * distance / (2.0 * low_cutoff * low_cutoff));
			}

            if (gaussian < 0) {
                gaussian = 0;
            }
            filter[y * width + x] = gaussian;

            // Update max_value if current gaussian is greater
            if (gaussian > max_value) {
                max_value = gaussian;
            }                
        }
    }
    // Normalize the filter to have maximum value of 1
...
template<typename T>
void construct_gaussian_lowpass_filter(T *filter, int width, int height, int low_cutoff_x, int high_cutoff_x, int low_cutoff_y, int high_cutoff_y) {
    int cx = width / 2;
    int cy = height / 2;

    T sigma_hx = high_cutoff_x / sqrt(2 * log(2));
    T sigma_lx = low_cutoff_x / sqrt(2 * log(2));
    T sigma_hy = high_cutoff_y / sqrt(2 * log(2));
    T sigma_ly = low_cutoff_y / sqrt(2 * log(2));

    T max_value = 0.0;  // Variable to find the maximum value in the filter
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            // Calculate the Gaussian filter for each direction
            T gaussian_x = exp(-dx * dx / (2.0 * sigma_hx * sigma_hx));
            T gaussian_y = exp(-dy * dy / (2.0 * sigma_hy * sigma_hy));

            // Combine the two directions
            T gaussian = gaussian_x * gaussian_y;
...
template<typename T>
void construct_gaussian_dip_filter(T *filter, int width, int height, int low_cutoff_x, int high_cutoff_x, int low_cutoff_y, int high_cutoff_y) {
    int cx = width / 2;
    int cy = height / 2;
...
void bandpass_filter(int nx,int nz,int x1,int x2,int z1,int z2,float* bgrad_pp,float* bgrad_ps){
	...
	float *k_filter,*gpu_pp,*gpu_ps;
	cudaMalloc((void**)&k_filter,nx*nz*sizeof(float));
	cudaMalloc((void**)&gpu_pp,nx*nz*sizeof(float));
	cudaMalloc((void**)&gpu_ps,nx*nz*sizeof(float));		
	dim3 grid((nz+32-1)/32,(nx+32-1)/32);
	dim3 block(32,32);
	dim3 grid2((nx*nz+1024-1)/1024);	
	dim3 block2(1024);	

	float *gauss_band = new float[nx*nz];
    // construct_gaussian_bandpass_filter(gauss_band, nz, nx, z1, z2);      // width-> z    height-> x            cutoff_x -> z    cutoff_z -> x   	
    // construct_gaussian_lowpass_filter(gauss_band, nz, nx, 0, z2, 0, x2);      // width-> z    height-> x            cutoff_x -> z    cutoff_z -> x   
	construct_gaussian_dip_filter(gauss_band, nz, nx, 0, z2, 0, x2);     // dip filter
	cudaMemcpy(k_filter,gauss_band,nx*nz*sizeof(float),cudaMemcpyHostToDevice);	
	// construct_filter<<<grid,block>>>(nx,nz,x1,z1,x2,z2,k_filter);

	cufftHandle plan2D_forward;
	cufftHandle plan2D_inverse;
	cufftPlan2d(&plan2D_forward,nx,nz,CUFFT_Z2Z);
	cufftPlan2d(&plan2D_inverse,nx,nz,CUFFT_Z2Z);	
	cufftDoubleComplex *fft_pp;
	cufftDoubleComplex *fft_ps;	
	cudaMalloc((void**)&fft_pp,nx*nz*sizeof(cufftDoubleComplex));	   
	cudaMalloc((void**)&fft_ps,nx*nz*sizeof(cufftDoubleComplex));	


	cudaMemcpy(gpu_pp,bgrad_pp,nx*nz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(gpu_ps,bgrad_ps,nx*nz*sizeof(float),cudaMemcpyHostToDevice);	

	R2Z<<<grid2,block2>>>(gpu_pp,gpu_ps,fft_pp,fft_ps,nx*nz);
	CUFFT_CALL(cufftExecZ2Z(plan2D_forward, fft_pp, fft_pp, CUFFT_FORWARD));  
	CUFFT_CALL(cufftExecZ2Z(plan2D_forward, fft_ps, fft_ps, CUFFT_FORWARD));  		

	fft_shift<<<grid,block>>>(fft_pp,nz,nx);
	fft_shift<<<grid,block>>>(fft_ps,nz,nx);		
	GPU_lowpass<<<grid,block>>>(fft_pp,fft_ps,k_filter,nx,nz);
	ifft_shift<<<grid,block>>>(fft_pp,nz,nx);		
	ifft_shift<<<grid,block>>>(fft_ps,nz,nx);	
	
	CUFFT_CALL(cufftExecZ2Z(plan2D_inverse, fft_pp, fft_pp, CUFFT_INVERSE));
	CUFFT_CALL(cufftExecZ2Z(plan2D_inverse, fft_ps, fft_ps, CUFFT_INVERSE));			
	GPU_divide<<<grid,block>>>(fft_pp,fft_ps,nx,nz);

	Z2R<<<grid2,block2>>>(gpu_pp,gpu_ps,fft_pp,fft_ps,nx*nz);  

	cudaMemcpy(bgrad_pp,gpu_pp,nx*nz*sizeof(float),cudaMemcpyDeviceToHost);
	cudaMemcpy(bgrad_ps,gpu_ps,nx*nz*sizeof(float),cudaMemcpyDeviceToHost);	

	cudaFree(k_filter);
	cudaFree(gpu_pp);
	cudaFree(gpu_ps);	

	cufftDestroy(plan2D_forward);
...
void IMAGE_SINGLESHOT(FD2DGPU_ELASTIC &fdtdgpu,const int sxnum,const int nDim,const int myid,float *record,float *record_x,int *gc,bool rbc,bool cpu_mem,int ntt,int iter, \
	int iter_round1, int maxiter,char* snapdir,	float *image_pp,float *image_ps,float *cpu_pp_grad_old,float *cpu_ps_grad_old,double &misfit,float *misfit_ot){
...
//////////////////////////////////////
	...
	int forward_born = 0; //only background wavefield

	if(iter==0){
		forward_born=0;
	}

	bool prism_rwi = false;

	bool double_scatter = false;
...
//	Lm 
	for(int it=0; it<fdtdgpu.nt; ++it)    
    	{
			...
			fdtdgpu.born_modeling(direction,forward_born);

			fdtdgpu.record_background_wave(it);
	...
	// float *record_z = new float[fdtdgpu.ntr*fdtdgpu.nt];
	// float *record_x = new float[fdtdgpu.ntr*fdtdgpu.nt];
...
///////////////////////////////////////////////////////////// L2 norm ////////////////////////////////////////////////////
//	Lm + Lm2- d 
	fdtdgpu.subtract(scale_size,prism_rwi);	
...
//					std::cout<<it<<std::endl;
			...
			fdtdgpu.born_modeling(direction_rec,6);			
			// fdtdgpu.born_modeling(direction_rec,7);


// backward wavefield
        	fdtdgpu.elastic_rotation_addS_bac(it,0,stack);
			fdtdgpu.born_modeling(direction,8);			
			// fdtdgpu.born_modeling(direction,9);


	 		cudaDeviceSynchronize();		

			fdtdgpu.image_gradient(0,double_scatter);
...
//	bool rbc = false;
	...
	int forward_born = 1;
	bool doublescatter = false;					//2nd scatter 
//	Lm 
	for(int it=0; it<fdtdgpu.nt; ++it)    
    	{
        	fdtdgpu.elastic_rotation_addS_for(it,0); 
	...
	float scale_size = 1.0;
//	Lm + Lm2 - d 
	fdtdgpu.subtract(scale_size,doublescatter);					// r  =  r - alpha*q
	cudaMemcpy(record_residual_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	cudaMemcpy(record_residual_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	
...
// alpha = gT*g/(gT*LTLg) = (LT*delta_d)T*(LT*delta_d)/() = delta_dT*(L*LT*delta_d)/()
	...
	cudaDeviceSynchronize();
	// fdtdgpu.scatter_add(scale_size);			// Lm + Lm2	  add 2nd scatter
	...
	// for(int i=0;i<fdtdgpu.ntr;i++)
	// for(int j=0;j<fdtdgpu.nt;j++){
	// 	test1 += record_scatter_z[i*fdtdgpu.nt+j];
	// 	test2 += record_residual_z[i*fdtdgpu.nt+j];									
	// }	
...
void CALCULATE_ALPHA_parabola_mb(FD2DGPU_ELASTIC &fdtdgpu,const int sxnum,const int nDim,const int myid,float *record,float *record_x,int *gc,bool rbc,bool cpu_mem,int ntt,int iter,\
	...
	float *misfit_step0_ot,float *misfit_step1_ot,float *misfit_step2_ot){

	fdtdgpu.FD_ELASTIC_initial(nDim);
	fdtdgpu.BK_ELASTIC_initial(nDim);	
	fdtdgpu.LSRTM_initial(nDim);

	float* cpu_vp_backup = new float[fdtdgpu.allnx*fdtdgpu.allnz];
	float* cpu_vs_backup = new float[fdtdgpu.allnx*fdtdgpu.allnz];
	float* cpu_vp_test = new float[fdtdgpu.allnx*fdtdgpu.allnz];
	float* cpu_vs_test = new float[fdtdgpu.allnx*fdtdgpu.allnz];

	cudaMemcpy(cpu_vp_backup,fdtdgpu.vel,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyDeviceToHost);
	cudaMemcpy(cpu_vs_backup,fdtdgpu.vel_s,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyDeviceToHost);

	fdtdgpu.bufferVpVsHtoD(fdtdgpu.cmin);
	...
	// int forward_born = 0;			//only background forward
	int forward_born = 0;			// only background forward
	bool doublescatter = false;					//2nd scatter 
//	Lm 
	for(int it=0; it<fdtdgpu.nt; ++it)    
	...
	for(int i=0;i<fdtdgpu.ntr;i++)
	for(int j=0;j<fdtdgpu.nt;j++){
		misfit_step0 += (record_residual_z[i*fdtdgpu.nt+j]*record_residual_z[i*fdtdgpu.nt+j] + record_residual_x[i*fdtdgpu.nt+j]*record_residual_x[i*fdtdgpu.nt+j]);									
	}	


	// cudaMemcpy(record_scatter_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	// cudaMemcpy(record_scatter_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	
	// OT(record_x,record,record_scatter_x,record_scatter_z,record_residual_x,record_residual_z,fdtdgpu.nt,fdtdgpu.ntr,misfit_step0);

	// memset(record_residual_x,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	// memset(record_residual_z,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));

	// float single_misfit_step0_ot;

	// fdtdgpu.OT_W2_CAL(&single_misfit_step0_ot);

	// *misfit_step0_ot += single_misfit_step0_ot;

	fdtdgpu.FD_ELASTIC_initial(nDim);
	fdtdgpu.BK_ELASTIC_initial(nDim);		
	fdtdgpu.LSRTM_initial(nDim);

	for(int i=0;i<fdtdgpu.allnx;i++)
	for(int j=0;j<fdtdgpu.allnz;j++)
	{
		cpu_vp_test[i*fdtdgpu.allnz + j]  = cpu_vp_backup[i*fdtdgpu.allnz + j]  -  step_1 * cpu_pp_grad_old[i*fdtdgpu.allnz + j];
		cpu_vs_test[i*fdtdgpu.allnz + j]  = cpu_vs_backup[i*fdtdgpu.allnz + j]  -  step_1 * cpu_ps_grad_old[i*fdtdgpu.allnz + j];		
	}

	cudaMemcpy(fdtdgpu.vel,cpu_vp_test,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.vel_s,cpu_vs_test,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	...
	for(int i=0;i<fdtdgpu.ntr;i++)
	for(int j=0;j<fdtdgpu.nt;j++){
		misfit_step1 += (record_residual_z[i*fdtdgpu.nt+j]*record_residual_z[i*fdtdgpu.nt+j] + record_residual_x[i*fdtdgpu.nt+j]*record_residual_x[i*fdtdgpu.nt+j]);									
	}	


	// cudaMemcpy(record_scatter_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	// cudaMemcpy(record_scatter_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	
	// OT(record_x,record,record_scatter_x,record_scatter_z,record_residual_x,record_residual_z,fdtdgpu.nt,fdtdgpu.ntr,misfit_step1);

	// memset(record_residual_x,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	// memset(record_residual_z,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));

	// float single_misfit_step1_ot;

	// fdtdgpu.OT_W2_CAL(&single_misfit_step1_ot);

	// *misfit_step1_ot += single_misfit_step1_ot;
	...
	for(int j=0;j<fdtdgpu.allnz;j++)
	{
		cpu_vp_test[i*fdtdgpu.allnz + j]  = cpu_vp_backup[i*fdtdgpu.allnz + j]  -  step_2 * cpu_pp_grad_old[i*fdtdgpu.allnz + j];
		cpu_vs_test[i*fdtdgpu.allnz + j]  = cpu_vs_backup[i*fdtdgpu.allnz + j]  -  step_2 * cpu_ps_grad_old[i*fdtdgpu.allnz + j];		
	}
	...
	for(int i=0;i<fdtdgpu.ntr;i++)
	for(int j=0;j<fdtdgpu.nt;j++){
		misfit_step2 += (record_residual_z[i*fdtdgpu.nt+j]*record_residual_z[i*fdtdgpu.nt+j] + record_residual_x[i*fdtdgpu.nt+j]*record_residual_x[i*fdtdgpu.nt+j]);									
	}	


	// cudaMemcpy(record_scatter_z,fdtdgpu.GPUrecord_scatter_z,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);
	// cudaMemcpy(record_scatter_x,fdtdgpu.GPUrecord_scatter_x,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float),cudaMemcpyDeviceToHost);	
	// OT(record_x,record,record_scatter_x,record_scatter_z,record_residual_x,record_residual_z,fdtdgpu.nt,fdtdgpu.ntr,misfit_step2);

	// memset(record_residual_x,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));
	// memset(record_residual_z,0,fdtdgpu.ntr*fdtdgpu.nt*sizeof(float));

	// float single_misfit_step2_ot;

	// fdtdgpu.OT_W2_CAL(&single_misfit_step2_ot);

	// *misfit_step2_ot += single_misfit_step2_ot;

	cudaMemcpy(fdtdgpu.vel,cpu_vp_backup,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.vel_s,cpu_vs_backup,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);


	delete[] cpu_vp_test;
	delete[] cpu_vs_test;	
	delete[] cpu_vp_backup;
	delete[] cpu_vs_backup;	
	delete[] record_residual_z;
	delete[] record_residual_x;
...
void ELSRTM_CALCULATE_ALPHA_parabola_mp(FD2DGPU_ELASTIC &fdtdgpu,const int sxnum,const int nDim,const int myid,float *record,float *record_x,int *gc,bool rbc,bool cpu_mem,int ntt,int iter,\
	...
	cudaMemcpy(cpu_vp_backup,fdtdgpu.image_pp_m,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyDeviceToHost);
	cudaMemcpy(cpu_vs_backup,fdtdgpu.image_ps_m,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyDeviceToHost);
...
//	bool rbc = false;
	...
	// int forward_born = 0;			//only background forward
	int forward_born = 1;			// background forward	and born forward
	bool doublescatter = false;					//2nd scatter 
//	Lm 
	for(int it=0; it<fdtdgpu.nt; ++it)    
    	{
			...
			fdtdgpu.born_modeling(direction,forward_born);
			// fdtdgpu.record_background_wave(it);			
			fdtdgpu.record_scatter_new(it,doublescatter);
    	}
	...
	cudaMemcpy(fdtdgpu.image_pp_m,cpu_vp_test,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.image_ps_m,cpu_vs_test,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	...
	cudaMemcpy(fdtdgpu.image_pp_m,cpu_vp_backup,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.image_ps_m,cpu_vs_backup,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
...
void ELSRTM_CALCULATE_Hessian_vector(FD2DGPU_ELASTIC &fdtdgpu,const int sxnum,const int nDim,const int myid,float *record,float *record_x,int *gc,bool rbc,bool cpu_mem,int ntt,int iter,\
	...
	cudaMemcpy(fdtdgpu.image_pp_m,grad_pp,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.image_ps_m,grad_ps,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
...
// backward wavefield
			...
			fdtdgpu.hessian_vector_product();
	...
	for(int it=0; it<fdtdgpu.nt; ++it)    
    	{
			...
			// if(sxnum==20&&it%500==0){
			// 	char tmps_bk[1024];
			// 	cudaMemcpy(bkp,fdtdgpu.b_vz,nDim*sizeof(float),cudaMemcpyDeviceToHost);
			// 	sprintf(tmps_bk, "%s/snap_FW_bvz_it%d_%d_%d.dat",snapdir,it,fdtdgpu.nxpml,fdtdgpu.nzpml);
...
// backward wavefield
			...
			fdtdgpu.hessian_vector_product_mb();
...
//calculate H*delta_m
	...
	cudaMemcpy(fdtdgpu.image_pp_m,delta_pp,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
	cudaMemcpy(fdtdgpu.image_ps_m,delta_ps,fdtdgpu.allnx*fdtdgpu.allnz*sizeof(float),cudaMemcpyHostToDevice);
...
//	bool rbc = false;
	...
	cudaMemset(GPU_theta_up,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_theta_down,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_omega_up,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_omega_down,0,2*N*nx*nt*sizeof(float));		
	cudaMemset(GPU_theta_left,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_theta_right,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_omega_left,0,2*N*nx*nt*sizeof(float));
	cudaMemset(GPU_omega_right,0,2*N*nx*nt*sizeof(float));		
...
// backward wavefield
			...
			fdtdgpu.hessian_vector_product_misfit();
